...
|-------|---------------|------------|--------|
| **1. Load Specification** | Retrieve refined issue | Linear/JIRA MCP tools | Specification loaded |
...
| **7. Implementation Approach** | Describe high-level approach and key dependencies | — | Approach documented |
| **8. Document Patterns** | Reference existing code | — | Pattern references |
| **9. Write Plan** | Default: update Linear/JIRA issue. `--local`: output plan as text only — do NOT write files (command handles file writes) | Linear/JIRA MCP update OR text output | Plan written or content returned to command |
...
- [ ] Phase 7: Implementation Approach (high-level approach described)
- [ ] Phase 8: Document Patterns (references to existing code)
- [ ] Phase 9: Write Plan — default: update Linear/JIRA issue; --local: output plan as text only (command writes the files, not this skill)
```
...
For comprehensive testing strategy examples with specific test cases and what NOT to test at each level, see **resources/testing-templates.md**
...
**Describe the high-level implementation approach — NOT a detailed breakdown.**

The Technical Plan should outline the general approach, key components, and critical dependencies. The detailed breakdown into vertical phases and horizontal tasks is done later by `/breakdown`.
...
- [Component X] must exist before [Component Y] (needs schema/interface/model)
- [Component Z] can be developed independently

**Suggested approach:**
[1-2 paragraphs describing the general order and rationale — e.g., "Start with the core
data model and a thin end-to-end slice, then extend with additional behaviors." Don't
prescribe detailed phases — `/breakdown` will slice this into vertical phases with
horizontal tasks.]
```

**IMPORTANT:** Do NOT create a detailed phase-by-phase breakdown here. That is the responsibility of `/breakdown`, which will:
- Slice the work into **vertical phases** (each delivering demoable e2e functionality)
- Break each phase into **horizontal tasks** by layer (DB, service, API, UI)
- Define acceptance criteria and acceptance tests per phase

The plan's job is to describe the technical approach, components, and dependencies so that `/breakdown` can make informed slicing decisions.

**For examples of implementation approach descriptions, see:** [resources/example-implementation-approach.md](resources/example-implementation-approach.md)
...
### Implementation Approach
[1-2 paragraphs: Key components, critical dependencies, general approach]
...
**Two output modes — check if `--local` was passed:**

#### Default mode: Write to Linear/JIRA issue
...
#### Local mode (`--local`): Return plan content — command writes the files

Do NOT update the Linear/JIRA issue.
Do NOT write any files yourself in this mode.

**Your job in local mode is to produce the plan content as output text only. The `/plan` command is responsible for writing both files to the correct paths.**

Output the plan in this exact format so the command can capture and write it:
...
## Specification Summary
[1 paragraph summarising the specification from the issue — the WHAT]
...
After outputting the plan content, state:

```
Plan content complete. Returning to /plan command to write files.
```

The command will then write:
1. The plan file at the path specified by `dev_state.plan_file` (e.g. `plans/plan-alm-122.md`) — the plan content above
2. `.agents/config.yaml` — the dev_state manifest
...
| **Approach not breakdown** | Describe implementation approach and key dependencies — leave detailed phasing to `/breakdown` |
...
- **Approach not breakdown** - Describe components and dependencies, leave detailed vertical phasing to `/breakdown`
- **High-level not code-level** - Focus on architecture, not implementation details
- **In `--local` mode: do NOT write any files** — output the plan as text and return. The `/plan` command writes the plan to `dev_state.plan_file` and `.agents/config.yaml`.
- **Default: all in PM system** — Technical Plan appended to Linear/JIRA issue; Linear is NOT touched in `--local` mode
...
- **resources/example-implementation-approach.md** - High-level implementation approach examples (components, dependencies, starting points — not detailed breakdowns)
